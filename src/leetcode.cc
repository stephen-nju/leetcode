...
int Solution::diameterOfBinaryTree(TreeNode *root) {
    std::function<int(TreeNode *, int *)> max_depth = [&](TreeNode *root,
                                                          int *diameter) {
        if (root == nullptr)
            return 0;
...
int Solution::maxProfit_3(vector<int> &prices) {
    ...
    vector<vector<vector<int>>> dp(n + 1,
                                   vector<vector<int>>(2 + 1, vector<int>(2)));
    ...
    for (int i = 1; i < n; i++) {
        for (int k = 1; k < K + 1; k++) {
            ...
            dp[i][k][0] =
                std::max(dp[i - 1][k][0], dp[i - 1][k][1] + prices[i]);
            dp[i][k][1] =
...
void backtracking_combination_sum3(int n, int k, int start,
                                   vector<vector<int>> &result,
                                   vector<int> &stack) {
    if (stack.size() == k &&
        std::accumulate(stack.begin(), stack.end(), 0) == n) {
...
void backtracking_letter_combination(int start, string &digits,
    ...
    for (int j = 0; j < letters.size(); j++) {
        ...
        backtracking_letter_combination(start + 1, digits, result, stack,
                                        letters_map);
        stack.pop_back();
    }
}
...
void backtracking_combination_sum(vector<int> &candidate, int target, int begin,
                                  vector<vector<int>> &result,
                                  vector<int> &stack) {
    if (target == 0) {
        result.push_back(stack);
...
void backtracking_combination_sum_2(vector<pair<int, int>> &freq, int pos,
...
vector<vector<int>> Solution::combinationSum2(vector<int> &candidates,
                                              int target) {
    // 方案一：考虑如何在递归的过程中进行去重
...
void backtracking_partition(const string &s, int pos,
...
void backtracking_restore_ip_addresses(const string &s, int pos,
                                       vector<string> &result,
                                       vector<string> &stack) {
    // 递归的终止条件
    if (pos >= s.size()) {
...
void backtracking_subsets(vector<int> &nums, int pos,
                          vector<vector<int>> &result, vector<int> &stack) {
    result.push_back(stack);
...
void backtracking_subsets_dup(vector<int> &nums, int pos,
...
void backtracking_find_subsequences(vector<int> &nums, int pos,
                                    vector<vector<int>> &result,
                                    vector<int> &stack) {
    // 至少需要两个元素
    if (stack.size() > 1) {
...
void backtracking_permute(vector<int> nums, vector<bool> &used,
                          vector<vector<int>> &result, vector<int> &stack) {
...
void backtracking_permute_unique(vector<int> &nums, vector<bool> &used,
                                 vector<vector<int>> &result,
                                 vector<int> &stack) {
    if (stack.size() == nums.size()) {
        result.push_back(stack);
...
vector<string> hierholzer_find_itinerary(vector<vector<string>> &tickets,
                                         string &vertex) {
    vector<string> result;
    // 先构建图(图的存储方案),欧拉半图
    std::unordered_map<string, std::priority_queue<string, vector<string>,
                                                   std::greater<string>>>
        g;
    for (auto t : tickets) {
        g[t[0]].push(t[1]);
    ...
    std::function<bool(int, int)> is_valid = [&](int row, int column) {
        ...
        for (int R = 0; R < row; R++) {
            ...
            if (column == C)
                return false;
            // 不能再同一斜列
            if ((row + column) == (R + C) || (R - C) == (row - column)) {
                return false;
            }
        }
        return true;
    ...
    std::function<void(int)> dfs_solve_n_queens = [&](int row) {
        ...
        if (row == n) {
            ...
            for (int i = 0; i < stack.size(); ++i) {
                // 注意单引号和双引号的区别
                board[i] =
                    string(stack[i], '.') + 'Q' + string(n - 1 - stack[i], '.');
            }
...
void Solution::solveSudoku(vector<vector<char>> &board) {
    ...
    // 用于判断当前位置填入的数是否有效
    std::function<bool(int row, int column, char c)> is_valid =
        [&](int row, int column, char c) {
            // 先判断是数字0-9
            if ((c - '0') <= 9 && (c - '0') > 0) {
                // 行
    ...
    std::function<bool(int row, int column)> backtracking_solve_sudoku =
        [&](int row, int column) {
            ...
            for (int i = 0; i < 9; i++) {
                if (is_valid(row, column, value[i])) {
                    ...
                    if (backtracking_solve_sudoku(row, column + 1))
                        return true;
                    // 错误需要回溯
                    board[row][column] = '.';
                }
...
bool Solution::isSymmetric(TreeNode *root) {
    ...
    std::function<bool(TreeNode *, TreeNode *)> dfs_compare =
...
vector<string> Solution::topKFrequent(vector<string> &words, int k) {
    ...
    std::priority_queue<std::pair<string, int>, vector<std::pair<string, int>>,
                        decltype(compare)>
        que(compare);
...
ListNode *Solution::mergeKLists(vector<ListNode *> &lists) {
    ...
    std::priority_queue<ListNode *, vector<ListNode *>, decltype(compare)>
        priority_que(compare);
...
int Solution::widthOfBinaryTree(TreeNode *root) {
    ...
    while (!que.empty()) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            temp.push_back(top);
            if (top.first->left)
                que.push(std::pair<TreeNode *, nlp_int64_t>(
                    top.first->left, 2 * (top.second - start)));
            if (top.first->right)
                que.push(std::pair<TreeNode *, nlp_int64_t>(
                    top.first->right, 2 * (top.second - start) + 1));
            que.pop();
        }
...
TreeNode *Solution::buildTree(vector<int> &inorder, vector<int> &postorder) {
    ...
    std::function<TreeNode *(vector<int> &, vector<int> &)> build =
        [&](vector<int> &in, vector<int> &post) -> TreeNode * {
        if (post.size() == 0)
            return nullptr;
        int root_value = post[post.size() - 1];
        TreeNode *node = new TreeNode(root_value);
        int delimiter_index;
        for (delimiter_index = 0; delimiter_index < in.size();
             delimiter_index++) {
            if (in[delimiter_index] == root_value)
                break;
        }
...
TreeNode *Solution::mergeTrees(TreeNode *root1, TreeNode *root2) {
    std::function<TreeNode *(TreeNode *, TreeNode *)> merge =
        [&](TreeNode *r1, TreeNode *r2) -> TreeNode * {
...
vector<int> Solution::dailyTemperatures(vector<int> &temperatures) {
    ...
    for (int index = 0; index < temperatures.size(); index++) {
        if (st.empty()) {
            ...
            while (!st.empty() &&
                   temperatures[index] > temperatures[st.top()]) {
                st.pop();
